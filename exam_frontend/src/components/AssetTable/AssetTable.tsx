...
const AssetTable = () => {
  ...
  const statusOptions = assetData?.data.results.map((item) => item.status) || [];
  const businessUnitOptions = assetData?.data.results.map((item) => item.business_unit.business_unit_name) || [];
  const locationOptions=assetData?.data.results.map((item)=>item.location.location_name)
...
if (assetData && assetData.data && assetData.data.results) {
  ...
  const allAssetCategories = assetData.data.results.map(asset => asset.asset_category);
