...
const TimelineDrawer: FC<{ assetUuid: string; onClose: () => void; open: boolean }> = ({ assetUuid, onClose, open }) => {
  return (
    <Drawer
...
const TimelineViewDrawer: FC<{ assetUuid: string }> = ({ assetUuid }) => {
  ...
  return (
    <>
      ...
      <TimelineDrawer assetUuid={assetUuid} onClose={handleDrawerClose} open={drawerOpen} />
    </>
  );
};
