    ...
    def list(self, request, *args, **kwargs):
        try:
            ...
            if assign_status == "unassigned":
                ...
                queryset = queryset.filter(Q(asset_detail_status="CREATED") | Q(asset_detail_status="UPDATED")|Q(asset_detail_status="CREATE_REJECTED")|Q(asset_detail_status="UPDATE_REJECTED"))
            
 
            # Apply additional filters if provided
            if asset_detail_status:
                # Split the status string by '|' to get individual statuses
                statuses = asset_detail_status.split('|')
                queryset = queryset.filter(asset_detail_status__in=statuses)
                # Apply filters based on individual statuses
