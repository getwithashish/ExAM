# import csv
# from asset.models import Asset, User, AssetType, BusinessUnit, Employee, Location, Memory
...
#     existing_assets = set(Asset.objects.values_list("asset_id", flat=True))
#     added_assets_count = 0
#     skipped_assets_count = 0

#     for row in csv_reader:
#         if row["asset_id"] in existing_assets:
#             skipped_assets_count += 1
#             continue  # Skip this row as it's already added
...
#         asset_type = AssetType.objects.filter(asset_type_name=row["asset_type_id"]).first()
...
#         existing_assets.add(row["asset_id"])
#         added_assets_count += 1

#     if added_assets_count == 0:
#         message = "No new data found. All files are duplicates."
#     else:
#         message = f"{added_assets_count} new data added to Asset table successfully. " \
