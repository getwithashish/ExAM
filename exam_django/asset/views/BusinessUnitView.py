...
class BusinessUnitView(ListCreateAPIView):
    def post(self, request, format=None):
        try:
            ...
            if serializer.is_valid():
                serializer.save()
                return Response({"data": serializer.data, "message": "Business unit created successfully"}, status=status.HTTP_201_CREATED)
    ...
    def get(self, request):
        try:
            ...
            return Response({"data": serializer.data, "message": "Business units details successfully retrieved"}, status=status.HTTP_200_OK)
        except Exception as e:
            return Response({"error": str(e)}, status=status.HTTP_500_INTERNAL_SERVER_ERROR)