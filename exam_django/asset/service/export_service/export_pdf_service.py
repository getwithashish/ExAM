...
class ExportPDF:
    ...
    def export_pdf(assets, expiry_dates):
        ...
        for i in range(0, num_assets, assets_per_page):
            ...
            page_expiry_dates = expiry_dates[
                i : i + assets_per_page
            ]  # Corresponding expiry dates for the current page
            ...
            for asset, expiry_date in zip(page_assets, page_expiry_dates):
                ...
                for field in fields:
                    ...
                    if field.lower() in [
                        ...
                        value = Paragraph(
                            str(value),
                            style=ParagraphStyle(name="Normal", wordWrap=True),
                        )
            ...
            transposed_data = [transposed_data[fields.index("Product_name")]] + [
                ...
                if i != fields.index("Product_name")
            ]

            # Calculate column widths based on content and available width
            col_widths = [
                max(len(str(cell)) * 8 for cell in col) for col in transposed_data
            ]
